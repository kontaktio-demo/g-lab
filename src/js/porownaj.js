...
(function () {
  ...
  function render() {
    ...
    function bar(v, max, cls) {
      ...
      return '<div class="bar"><div class="bar-fill ' + (cls||'') + '" style="width:' + p + '%"></div><span class="bar-val">' + v + '</span></div>';
    }

    var rows = [
      { label: 'Sterownik (ECU)', value: function(c){ return c.sterownik || '-'; } },
      { label: 'Pojemność', value: function(c){ return c.pojemnosc ? c.pojemnosc.toFixed(1) + ' l' : '-'; } },
      { label: 'Lata produkcji', value: function(c){ return (c.generacja||'') + ' (' + c.rok_od + '-' + c.rok_do + ')'; } },
      ...
      { label: 'Zmiana procentowa', value: function(c){ return c.diff_pct + '%'; } },
    ];

    var headRow = '<tr><th></th>' + picked.map(function (c) {
      return '<th><a href="/tuning/' + encodeURIComponent(c.slug) + '">' +
        c.marka + ' ' + c.model + '<br><small>' + c.silnik + '</small></a></th>';
    }).join('') + '</tr>';

    var bodyRows = rows.map(function (r) {
      return '<tr><th>' + r.label + '</th>' + picked.map(function (c) {
    ...
    box.innerHTML =
      ...
      '<div class="compare-cta"><a class="btn btn-primary" href="/wycena/?slug=' + encodeURIComponent(picked[0].slug) + '">Wyceń tuning ' + picked[0].marka + ' ' + picked[0].model + '</a></div>';
  }
