...
// Loads /data/katalog.json once, finds the car by ?slug=..., and renders
// title, power table, dyno chart, spec table, related links and Stage 1/2/3.
// This page is used for synthetic catalog entries (~20 000) which would be
// wasteful to serve as static HTML. Verified entries continue to use their
// own statically-generated /tuning/{slug}.html page.
...
(function () {
  ...
  function carUrl(c) {
    return c.s ? '/tuning/?slug=' + encodeURIComponent(c.slug) : '/tuning/' + c.slug;
  }
  ...
  function render(car, all) {
    ...
    var banner = $('auto-estimate-banner');
    if (banner) banner.hidden = !car.s; // pokaż tylko dla syntetyków
  ...
  function init() {
    ...
    getData().then(function (all) {
      ...
      // Verified mają własną statyczną stronę - jeśli ktoś trafił tu z linkiem
      // do verified, przekierowujemy na ładny URL.
      if (!car.s) {
        location.replace('/tuning/' + car.slug);
        return;
      }
      render(car, all);
    });
